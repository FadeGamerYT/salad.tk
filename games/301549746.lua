...
repeat RunService.RenderStepped:Wait() until game:IsLoaded()
  ...
  repeat wait() until Models ~= nil
    ...
    repeat wait() until ChrModels ~= nil
      ...
      local gui = library:New("salad.tk v2.98")
      local legit = gui:Tab("legit")
      local rage = gui:Tab("rage")
      local aatab = gui:Tab("anti-aim")
      local visuals = gui:Tab("visuals")
      local misc = gui:Tab("misc")
      local misc = gui:Tab("settings")
...
local luascripts = settings:Sector("lua scripts", "Left")
luascripts:Element("Scroll", "lua", {options = allluas, Amount = 5})
luascripts:Element("Button", "load", {}, function()
loadstring(readfile("saladlua\\"..values.luas["lua scripts"].lua.Scroll))()
end)
...
local configs = configuration:Sector("configs", "Left")
configs:Element("TextBox", "config", {placeholder = "config name"})
configs:Element("Button", "save", {}, function() if values.configuration.configs.config.Text ~= "" then library:SaveConfig(values.configuration.configs.config.Text) end end)
configs:Element("Button", "load", {}, function() if values.configuration.configs.config.Text ~= "" then ConfigLoad:Fire(values.configuration.configs.config.Text) end end)
...
if tbl.Toggle then
LocalPlayer.Cash.Value = "salad.tk on top"
end
end)
...
LocalPlayer.Character.Humanoid.HipHeight = values.rage.angles["high pos"].Slider or 3.5
else
